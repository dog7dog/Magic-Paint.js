...
function applyAnimationTransform(owner, center, cur, progress) {
  ...
  const kfDx = kfP && Number.isFinite(kfP.x) ? kfP.x - center.x : 0;
  const kfDy = kfP && Number.isFinite(kfP.y) ? kfP.y - center.y : 0;
...
cv.addEventListener('dblclick', e => {
  ...
  if (selected) {
    ...
    setStatus((selected.groupId ? 'グループ' : selected.name) + ' にパスを設定しました（' + pathPoints.length + '点）');
  } else {
...
const addKfFn = () => {
  ...
  const props = {
    opa: selected.opa ?? animOwner.opa ?? 100,
    rot: selected.rot ?? animOwner.rot ?? 0,
    color: selected.color || animOwner.color,
    x: c.x,
    y: c.y
  };
  const existing = animOwner.keyframes.find(k => Math.abs(k.t - t) < 0.01);

  if (existing) {
    existing.props = props;
  } else {
    animOwner.keyframes.push({ t, props });
  }
  ...
  setStatus((existing ? 'KF更新: ' : 'KF追加: ') + t.toFixed(2) + 's');
  toast('ti-diamond', t.toFixed(2) + 's にキーフレーム' + (existing ? '更新' : '追加'));
};
  ...
  getEditorAnimationItems().forEach(item => {
    ...
    if (kfs.length >= 1) {
      ...
      const propsFor = k => {
        ...
        if (Number.isFinite(x)) props.push('x: ' + (x - ctr.x).toFixed(2));
        if (Number.isFinite(y)) props.push('y: ' + (y - ctr.y).toFixed(2));
        return props.join(', ');
      };
      let code = 'const ' + tlName + ' = gsap.timeline({ repeat: ' + loopVal + ' });\n' +
        tlName + ".set('" + sel + "', { " + propsFor(sorted[0]) + ' });';
      for (let i = 1; i < sorted.length; i++) {
        const dur = Math.max(0, sorted[i].t - sorted[i - 1].t).toFixed(2);
        code += '\n' + tlName + ".to('" + sel + "', { duration: " + dur + ", ease: '" + ease + "', " + propsFor(sorted[i]) + ' });';
      }
      anims.push(code);
    }
