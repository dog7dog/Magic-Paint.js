...
const addKfFn = () => {
  ...
  const props = animationPropsForShape(selected);
  if (!props) return;

  if (!hasUserKeyframes(animOwner) && t > 0.01) {
    const baseProps = animOwner._kfBaseProps || props;
    const holdGap = Math.max(0.001, totalDur / 10000);
    const holdT = Math.max(0, t - holdGap);
  ...
  const existing = animOwner.keyframes.find(k => !k.autoHold && Math.abs(k.t - t) < 0.01);
...
function setPlaybackButtonState(playing) {
  ...
  if (runBtn) {
    runBtn.innerHTML = playing
      ...
      : '<i class="ti ti-player-play"></i> 実行';
...
function initJSEditor() {
  ...
  runBtn.title = '再生: 通常はキャンバス再生 / 手書きJSはGSAP実行';
