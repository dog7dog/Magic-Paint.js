...
function applyAnimationTransform(owner, center, cur, progress) {
  ...
  const pos = getPathPos(progress, owner.animPath || null);

  if (pos && owner.animPath && owner.animPath[0]) {
    ctx.translate(pos.x - owner.animPath[0].x, pos.y - owner.animPath[0].y);
  }

  if (kfP) {
    ctx.translate(center.x, center.y);
    ctx.rotate(((kfP.rot - (owner.rot || 0)) * Math.PI) / 180);
    ctx.translate(-center.x, -center.y);
  }
...
function drawAnimatedScene(cur, progress) {
  ...
  shapes.forEach(s => {
    ...
    ctx.save();
    const kfP = applyAnimationTransform(s, center, cur, progress);
...
function paste() {
  ...
  copy.keyframes = []; copy.animPath = null;
  ...
  else if (copy.type === 'pen') { copy.pts = copy.pts.map(p => ({ x: p.x + 20, y: p.y + 20 })); }
...
function setTool(t) {
  ...
  if (t === 'path' && selected) {
    setStatus(selected.name + ' → ドラッグしてパスを描いてください');
  } else if (t === 'path') {
...
cv.addEventListener('dblclick', e => {
  ...
  if (selected) {
    selected.animPath = [...pathPoints];
    markGroupAnimationOwner(selected);
    setStatus(selected.name + ' にパスを設定しました（' + pathPoints.length + '点）');
  } else {
...
function syncProps() {
  ...
  set('p-auto-rotate', animOwner?.autoRotate ?? 0, '°/s');
}
...
[
  ...
  ['p-auto-rotate', 'p-auto-rotate-v', '°/s', v => {
    const animOwner = getSelectedAnimationOwner();
    if (animOwner) {
      animOwner.autoRotate = v;
...
document.getElementById('btn-clear-anim').addEventListener('click', () => {
  ...
  const animOwner = getSelectedAnimationOwner();
  if (!animOwner) return;
  animOwner.animPath = null; animOwner.keyframes = []; animOwner.autoRotate = 0;
...
trackCv.addEventListener('click', e => {
  ...
  if (idx >= 0 && idx < shapes.length) {
    ...
    selected = shapes[idx]; syncProps(); syncLayers(); redraw(); drawTimeline();
  }
});
...
const addKfFn = () => {
  ...
  if (animOwner.keyframes.find(k => Math.abs(k.t - t) < 0.01)) { setStatus('このフレームには既にKFがあります'); return; }
  const c = getAnimationCenter(selected);
  animOwner.keyframes.push({ t, props: { opa: selected.opa || 100, rot: selected.rot || 0, color: selected.color, x: c.x, y: c.y } });
  markGroupAnimationOwner(animOwner);
  animOwner.keyframes.sort((a, b) => a.t - b.t);
  drawTimeline(); updateCode();
  toast('ti-diamond', t.toFixed(2) + 's にキーフレーム追加');
};
...
function interpKF(kfs, t) {
  ...
  if (!before.length) return { ...sorted[0].props };
  if (!after.length) return { ...sorted[sorted.length - 1].props };
  const k0 = before[before.length - 1], k1 = after[0];
  const f = (t - k0.t) / (k1.t - k0.t);
  return {
    opa: k0.props.opa + (k1.props.opa - k0.props.opa) * f,
    rot: k0.props.rot + (k1.props.rot - k0.props.rot) * f,
...
function animStep(ts) {
  ...
  const cur = animT * totalDur;
  ctx.clearRect(0, 0, cv.width, cv.height);
  ctx.fillStyle = canvasBg;
  ctx.fillRect(0, 0, cv.width, cv.height);

  drawAnimatedScene(cur, animT);

  // パスオーバーレイ
  shapes.forEach(s => {
    if (!s.animPath || s.animPath.length < 2) return;
    ctx.save(); ctx.strokeStyle = s.color; ctx.lineWidth = 1.5;
    ctx.setLineDash([6, 4]); ctx.globalAlpha = 0.35;
    ctx.beginPath(); ctx.moveTo(s.animPath[0].x, s.animPath[0].y);
    s.animPath.forEach(p => ctx.lineTo(p.x, p.y)); ctx.stroke();
...
function startAnim() {
  ...
  document.getElementById('tl-play').innerHTML = '<i class="ti ti-player-pause"></i>';
...
function stopAnim() {
  ...
  document.getElementById('tl-play').innerHTML = '<i class="ti ti-player-play"></i>';
...
` +
    ...
    if (kfs.length >= 2) {
      const sorted = [...kfs].sort((a, b) => a.t - b.t);
      const kfStr = sorted.map(k =>
        `  { duration: ${k.t.toFixed(2)}, opacity: ${(k.props.opa / 100).toFixed(2)}, rotation: ${k.props.rot} }`
      ).join(',\n');
      anims.push(
        `gsap.timeline({ repeat: ${loopVal} })
` +
        `  .to('${sel}', [
${kfStr}
  ]);`
...
` +
    ...
    if (anims.length > 0) {
      lines.push(`// ${s.name || s.type} (index ${i})`);
      lines.push(...anims);
      lines.push('');
    }
  });
...
function stopJeAnim() {
  if (typeof gsap !== 'undefined') {
    try { gsap.globalTimeline.pause(); gsap.globalTimeline.clear(); } catch (e) { }
  }
  // SVGオーバーレイを必ず削除
  if (_jeSvg) { _jeSvg.remove(); _jeSvg = null; }
...
function buildSVGContent() {
  // GSAPのmotionPathはtransformを動かす
  // → 全図形を g[transform="translate(cx,cy)"] + 原点中心の形状 で表現する
  // → motionPathにパス絶対座標を渡せば正しくなぞる
  return shapes.map((s, i) => {
    ...
    if (s.type === 'rect') {
      ...
      inner = `<rect x="${Math.round(-hw)}" y="${Math.round(-hh)}" ` +
        `width="${Math.round(s.w)}" height="${Math.round(s.h)}" rx="${s.rr || 0}" ` +
        `fill="${fi}" stroke="${c}" stroke-width="${sw2}" ${dd}/>`;
    } else if (s.type === 'circle') {
      inner = `<ellipse cx="0" cy="0" ` +
        `rx="${Math.round(s.rx)}" ry="${Math.round(s.ry)}" ` +
        `fill="${fi}" stroke="${c}" stroke-width="${sw2}"/>`;
      ...
      inner = `<line x1="${Math.round(s.x1 - mx)}" y1="${Math.round(s.y1 - my)}" ` +
        `x2="${Math.round(s.x2 - mx)}" y2="${Math.round(s.y2 - my)}" ` +
        `stroke="${c}" stroke-width="${sw2}" ${dd}/>`;
      ...
      if (renderer?.toSVG) {
        inner = renderer.toSVG(s);
      }
    }

    // ブラシは snap 画像を data URL として埋め込む
    if (s.type === 'brush' && s.snap) {
      try {
        const dataUrl = s.snap.toDataURL('image/png');
        return `<image class="${cls}" href="${dataUrl}" x="0" y="0" width="${s.snap.width}" height="${s.snap.height}" opacity="${op}"/>`;
      } catch { return ''; }
    }

    if (!inner) return '';

    // g要素で中心にtranslateして配置
    return `<g class="${cls}" transform="translate(${cx},${cy})" opacity="${op}">${inner}</g>`;
  }).filter(Boolean).join('\n  ');
}
